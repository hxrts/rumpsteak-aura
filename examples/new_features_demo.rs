...
fn demo_multiple_annotations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Protocol::Send { annotations, from_annotations, to_annotations, .. } = &choreo.protocol {
...
fn demo_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for role in &choreo.roles {
        match project(&choreo, role) {
            Ok(_) => projection_count += 1,
            Err(_) => {} // Expected for some dynamic roles
        }
    }
    ...
    println!("   • Generated {} lines of Rust code", code_str.lines().count());
